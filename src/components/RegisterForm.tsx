...
const RegisterForm: React.FC<{ switchToLogin: () => void }> = ({ switchToLogin }) => {
  ...
  const onSubmit = async (data: RegisterFormValues) => {
    ...
    try {
      ...
      // After signup, we would normally save additional user data to Firestore
      // Example: await addDoc(collection(db, "users"), { name: data.name, email: data.email });
    ...
    <div className="w-full max-w-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <button
          ...
          className="w-full py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg font-medium flex items-center justify-center gap-2 hover:opacity-90 transition-opacity disabled:opacity-70 disabled:cursor-not-allowed group mt-6"
        >
